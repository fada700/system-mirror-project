...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE staff uuid := public.current_usuario_id(); s solicitudes%ROWTYPE; num text; cvv text; venc text;
BEGIN
  ...
  UPDATE tarjetas_credito SET estado='activa', numero=num, cvv=cvv, vencimiento=venc, limite=COALESCE(limite,5000)
