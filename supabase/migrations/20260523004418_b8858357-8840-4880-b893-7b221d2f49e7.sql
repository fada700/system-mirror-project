...
DECLARE uid uuid := public.current_usuario_id(); v_role public.app_role; v_monto numeric; v_dias int; ultimo timestamptz; owner uuid; owner_saldo numeric;
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'No autenticado'; END IF;
...
BEGIN
  IF NOT (public.has_role('admin')) THEN RAISE EXCEPTION 'No autorizado'; END IF;
  owner := public.dueno_usuario_id();
  FOR r IN
...
-- Lock down execution of these functions to authenticated only
REVOKE EXECUTE ON FUNCTION public.emitir_multa(uuid, numeric, text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.pagar_multa(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.cancelar_multa(uuid, text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.marcar_recordatorio_multa(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.reclamar_sueldo() FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.proximo_sueldo() FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.comprar_membresia(public.tipo_membresia) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.cobrar_impuestos_tick() FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.check_limite_transaccion(uuid, numeric) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.cobrar_impuestos_tick() TO authenticated;
GRANT EXECUTE ON FUNCTION public.check_limite_transaccion(uuid, numeric) TO authenticated;