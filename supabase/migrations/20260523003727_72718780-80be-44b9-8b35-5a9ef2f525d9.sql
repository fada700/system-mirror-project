-- Fix: aprobar_tarjeta_credito defensivo
...
BEGIN
  ...
  IF td.id IS NOT NULL AND td.estado <> 'cerrada' THEN RAISE EXCEPTION 'Ya tiene tarjeta débito activa'; END IF;
...
BEGIN
  ...
  INSERT INTO movimientos(usuario_id, tipo, monto, descripcion)
    VALUES (u.id, (CASE WHEN _delta > 0 THEN 'admin_dar' ELSE 'admin_quitar' END)::tipo_movimiento, abs(_delta),
  ...
  PERFORM public.log_audit(CASE WHEN _delta > 0 THEN 'ADD_BALANCE' ELSE 'REMOVE_BALANCE' END,
    'usuario', u.id, u.nombre, jsonb_build_object('cuenta', _cuenta, 'delta', _delta, 'motivo', _motivo));
END;
$function$;
...
BEGIN
  ...
  SELECT * INTO u FROM usuarios WHERE id = _usuario_id;
  PERFORM public.log_audit('AJUSTAR_LIMITE','tarjeta_credito', _usuario_id, u.nombre, jsonb_build_object('antes', antes, 'despues', _nuevo_limite));
END;
$function$;
...
DECLARE uid uuid := public.current_usuario_id(); est public.estado_cuenta_general; tc tarjetas_credito%ROWTYPE; sol_id uuid;
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'No autenticado'; END IF;
  SELECT estado_cuenta INTO est FROM usuarios WHERE id = uid;
